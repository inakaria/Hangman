...
# Name: Milena Musiienko
# Collaborators: Prisych Kostiantyn
# Time spent: ~ 10 hours
...
def is_word_guessed(secret_word, letters_guessed):
    ...
    returns: boolean, True if all the letters of secret_word are in letters_guessed;
      False otherwise
    '''
    setword=set(secret_word)
    setletter=set(letters_guessed)

    if len(setletter)-len(setword)==0:
        return True
    else:
        return False
...
def get_available_letters(letters_guessed):
    ...
    all_letters_dc=copy.deepcopy(all_letters)
    for letter in all_letters_dc:
        if letter in letters_guessed:
    ...
    * After each guess, you should display to the user the 
    ...
    while True:
      ...
      if len(letter)==0:
        if warnings>0:
          warnings-=1
          print(f'Oops! Empty input. You have {warnings} warnings left: {get_guessed_word(secret_word, letters_guessed)}')
        else:
          guesses-=1
          print(f'Oops! Empty input. You have no warnings left so you lose one guess: {get_guessed_word(secret_word, letters_guessed)}')
      ...
      if guesses<=0 or is_word_guessed(secret_word, letters_guessed)==True:
        break
...
def match_with_gaps(my_word, other_word):
    ...
    if len(my_word)!=len(other_word):
      return False
    for letter in range(0, len(other_word), 1):
...
def show_possible_matches(my_word):
    ...
    for word in wordlist:
        if match_with_gaps(my_word, word):
            find=my_word.replace('_', '[a-z]')
            if re.match(find, word):
                    suitable_words.append(word)
    if len(suitable_words)==0:
        return 'No matches found'
    else:
        return ' '.join(suitable_words)
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while True:
      ...
      elif letter not in string.ascii_lowercase and not '*':
        if warnings>0:
          warnings-=1
          print(f'Oops! That is not a valid letter. You have {warnings} warnings left: {get_guessed_word(secret_word, letters_guessed)}')
        else:
...
###############
    ...
    #secret_word = choose_word(wordlist)
    #hangman_with_hints(secret_word='tart')
